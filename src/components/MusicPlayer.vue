...
const { isPlaying, audio, currentTrack, currentArtist } = storeToRefs(song);
...
watch(
  ...
  (time) => {
    if (time && time == isTrackTimeTotal.value) {
      song.nextSong(currentTrack.value);
    }
  }
);
</script>
        ...
        <div class="flex items-center justify-center h-[30px]">
          <button class="mx-2">
            <SkipBackward
              fillColor="#FFFFFF"
              :size="25"
              @click="song.prevSong(currentTrack)"
            />
          </button>
          <button
            class="p-1 rounded-full mx-3 bg-white"
            @click="song.playOrPauseThisSong(currentArtist, currentTrack)"
          >
          ...
          <button class="mx-2">
            <SkipForward
              fillColor="#FFFFFF"
              :size="25"
              @click="song.nextSong(currentTrack)"
            />
          </button>
        </div>
      </div>
